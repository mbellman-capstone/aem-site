(function() {
	...
	// interact with.
	var selectors = {
			self:      '[data-cmp-is="pricing-banner"]'
	};

	function PricingBanner(config) {
			...
			function init(config) {
					...
					window.addEventListener("scroll", function() {
						...
						var image = root.querySelector('.c-pricing-banner__background img');
	...
	function onDocumentReady() {
			...
			for (var i = 0; i < elements.length; i++) {
					new PricingBanner({ element: elements[i] });
			}
									...
									nodesArray.forEach(function(addedNode) {
											if (addedNode.querySelectorAll) {
													...
													elementsArray.forEach(function(element) {
															new PricingBanner({ element: element });
													});
											}
									});
							}
